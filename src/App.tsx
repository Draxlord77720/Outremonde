...
function Rules() {
  const rules = [
    ...
    ['🧭 Déclenchements liés au jeu d’une carte', 'Pour une carte jouée en Spirituel, le Bonus d’affinité puis l’effet Spirituel propre se résolvent avant les autres effets qui se sont déclenchés uniquement parce que cette carte a été jouée. Les effets déclenchés pendant la résolution du Bonus ou de l’effet propre sont traités après l’effet qui les a créés.'],
    ...
    ['⏱ Déclenchements d’attaque et de blocage', 'Après la déclaration simultanée des attaquants et de leur ordre de résolution, résolvez les effets déclenchés par « lorsqu’elle attaque » avant la déclaration des bloqueurs. Après la déclaration simultanée des bloqueurs, résolvez les effets déclenchés par le blocage avant de résoudre la première attaque. Si plusieurs événements éligibles surviennent simultanément pour un effet disant « la première fois », le contrôleur de cet effet choisit l’événement ou la créature concernée.'],
    ['↔ Événements simultanés', 'Si plusieurs effets contrôlés par un même joueur se déclenchent simultanément, ce joueur choisit leur ordre de résolution. Si les deux joueurs ont des effets déclenchés au même moment, le joueur actif résout d’abord les siens dans l’ordre de son choix, puis l’autre joueur résout les siens. Si plusieurs créatures meurent simultanément, elles quittent toutes le Terrain Physique avant la résolution de leurs Réincarnations.'],
    ['🛡 Interposition', 'Un attaquant peut être bloqué par un seul bloqueur et un bloqueur ne peut normalement bloquer qu’une seule attaque. Si le bloqueur n’est plus sur le Terrain Physique au moment où cette attaque doit se résoudre, l’attaque devient non bloquée et l’attaquant inflige normalement son ATK au joueur adverse.'],
    ['💥 Combat', 'Les attaques sont résolues une par une dans l’ordre annoncé. Seule la créature ATTAQUANTE inflige des dégâts : le bloqueur ne riposte jamais. Sans bloqueur, l’attaquant inflige son ATK au joueur adverse. Si l’attaquant n’est plus sur le Terrain Physique au moment où son attaque doit se résoudre, cette attaque ne se résout pas.'],
    ['📊 ATK, DEF et mort', 'Les effets qui mentionnent ATK ou DEF utilisent leur valeur actuelle, après modificateurs, sauf si le texte précise « imprimé ». Une ATK inférieure à 0 est traitée comme 0 pour infliger des dégâts. Après la résolution complète d’un effet ou d’un événement de dégâts, une créature Physique meurt si sa DEF est de 0 ou moins, ou si les dégâts marqués sur elle sont au moins égaux à sa DEF actuelle.'],
    ['🔥 Impact X', 'Si une attaque avec Impact X est bloquée et que ses dégâts détruisent le bloqueur, les dégâts excédentaires réellement disponibles après les augmentations, Protection, Armure et les dégâts déjà marqués sur ce bloqueur peuvent être infligés au joueur adverse, jusqu’à un maximum de X. Si le bloqueur a quitté le Terrain Physique avant la résolution, l’attaque est non bloquée : elle inflige normalement l’ATK de l’attaquant au joueur adverse et Impact ne s’applique pas.'],
    ['➕ Plusieurs valeurs d’Impact', 'Si une même créature possède plusieurs valeurs d’Impact X, elles ne s’additionnent pas : seule la valeur la plus élevée s’applique à cette attaque.'],
    ...
    ['➕ Plusieurs valeurs de Drain', 'Si une même créature possède plusieurs valeurs de Drain X, elles ne s’additionnent pas : seule la valeur la plus élevée s’applique. Drain ne peut se déclencher qu’une seule fois par créature et par tour.'],
    ...
    ['📚 Deck presque vide', 'Lorsqu’un effet regarde, révèle ou met au cimetière les X cartes du dessus d’un deck et que ce deck en contient moins de X, faites autant que possible avec les cartes restantes. Mettre des cartes du dessus du deck au cimetière n’est pas piocher : cela ne provoque pas immédiatement une défaite ; la défaite survient lorsqu’un joueur doit réellement piocher alors que son deck est vide.'],
